...
void test_iterator() {
	node_p f1 = node_alloc(NT_FUNC_DEF);
		node_p i11 = node_alloc_append(NT_ARG, f1, &f1->func_def.in);
		node_p i12 = node_alloc_append(NT_ARG, f1, &f1->func_def.in);
		
		node_p o11 = node_alloc_append(NT_ARG, f1, &f1->func_def.out);
		node_p o12 = node_alloc_append(NT_ARG, f1, &f1->func_def.out);
		node_p o13 = node_alloc_append(NT_ARG, f1, &f1->func_def.out);
		
		node_p b11 = node_alloc_append(NT_VAR, f1, &f1->func_def.body);
		node_p b12 = node_alloc_append(NT_VAR, f1, &f1->func_def.body);
		...
		node_p b13 = node_alloc_append(NT_IF_STMT, f1, &f1->func_def.body);
...
void test_ast_replace_node() {
	...
	it = ast_start(f1);
		...
		st_check(f1->func_def.in.ptr[0] == r1);
		st_check(f1->func_def.in.ptr[1] == i12);
	it = ast_next(f1, it);
		st_check(it.node == i12);
		ast_replace_node(f1, it, r2);
		st_check(it.node == i12);
		st_check(f1->func_def.in.ptr[0] == r1);
		st_check(f1->func_def.in.ptr[1] == r2);
}
