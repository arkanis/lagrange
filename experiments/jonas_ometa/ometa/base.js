/// base_new
// - just added function-names to _not and _or to allow hooking to some internal function-calls
// - and exported global
// - also changed `objectThatDelegatesTo` to more intuitive `Object.inherit`
// - cleaned up a little and run through jsbeautifier.org
// - removed `tempnam` because it hasn't been used
// - now using builtin functions where possible
// - removed method `at` from string and array and used native bracket access instead
//   this may cause problems, because index access to not exsisting char returns undefined
//   instead of empty-string.
// - removed ReadStream
// - removed _fromIdx and $elf
//
// TODO
// remove _fromIdx from grammar
// rewrite locals template in BSOmetaTranslator
// only declare variables in locals unique (not "var t,t,t,t,t,t,t")
// allow memoization of parametrized rules

/**
 * Old runtime: all tests passed within ~9s
 * New runtime: all tests passed within ~18s
 * 
 * Reusing error to throw does not affect performance
 */



/**
 * 1. Add methods to Object & Array
 */
 ...
 *
 * While the backtracker implementation provides the internal parser-functionality
 */
var OMeta = (function() {
  ...
  function Backtracker(input) {
     ...
     *                                 object
     *
     * Not slicing the stream brought performance back to ~5s  (from 9)
     *
     * @return Stream
     */
    function Stream(streamable, rest_or_pos) {
      ...
      // TODO make double sure, that this position thing works
    ...
    // QUESTION: Are currently failures stored in the memoizer to be reraised?
      ...
      try { return rule.call(this); } 
        ...
        this.bt.catch_mismatch(e, 
          ["Could not apply ", ruleName, 
           " with args [" + [].slice.call(arguments, 1).join(', '), "]"].join(''));
      }*/
    },
